...
1. **Check `huitzo.yaml` exists and is valid YAML**. Read it and verify required fields:
   ...
   - Each command has `entry_point` field (e.g., `module.commands.verb_noun:verb_noun`)
...
6. **Check `pyproject.toml`** is auto-generated:
   - File starts with `# AUTO-GENERATED` header (never manually edited)
   - `project.name` matches pack name
   - `huitzo-sdk` is in dependencies
   - Entry points match the `entry_point` fields in `huitzo.yaml` (generated by `huitzo pack sync`)
